...
7. チェック結果を生成する。
8. 開催日時の降順にソートする。
9. ログイン済みのイベント一覧を取得できた場合は、最新のCookieを `storageState.json` へ保存する。
10. コンソールとSlackへ結果を出力する。

前回正常取得件数は `logs/last-successful-event-count.json` に保存する。前回が10件以上かつ今回が前回の50%未満の場合は、一覧取得異常として処理を中断する。
...
### 3.2.1 `src/check-state.ts`

前回正常取得時のイベント件数を読み書きし、取得件数の急減を検出する。
...
イベント名からイベント種別を判定する。
イベント名に地域マーカー、`文学フリマ`、または `猫町.で、旅をしよう` を含む場合は `offline` と判定する。

### 3.10 `src/utils/date.ts`

日本語日時文字列の解析、締切時刻抽出、開始5分前判定、受付開始時刻抽出を担当する。
...
- プラン変更チケット以外でオンライン開催ONのチケットが1件以上存在すること: イベント単位
...
- ページ本文内の `受付開始` 時刻取得: イベント単位
- ページ本文の `受付開始` 時刻と主催者お知らせ内の受付開始時刻の一致: イベント単位
- ページ本文の `参加費` に `1回目` または `2回目` がある場合の `今月1回目` と `今月2回目以降` の表記: イベント単位。どちらもなければチェック対象外
...
受付開始時刻は `extractReceptionStartTimeFromBody` と `extractReceptionStartTimeFromNotice` で抽出する。
ページ本文では `20:00 受付開始` のような明示表記を読む。
主催者お知らせでは `20:00から受付を開始` のような明示表記と、イベント開始日時を基準にした `読書会スタート30分前から受付` のような相対表記を読む。
ページ本文から受付開始時刻を取得できる場合のみ一致チェックをイベント単位で1回実施する。
...
イベント名に `駒井さんと読む` または `駒井稔さんと読む` を含み、`ゲスト` を含まない場合は、`さんと読む` によるゲスト判定から除外する。
...
プラン変更チケットが1つあり、それ以外のチケットが1つの場合は固定費イベントとみなす。プラン変更チケット以外が複数の場合は、各チケットがオン/オフ/ハイ/外を含めば固定費イベントとみなす。各固定費チケットの金額は、イベント種別の通常料金として許可された金額のいずれかである必要がある。許可範囲内であれば、チケット間で金額が異なっていてもよい。チケットが2つで片方がプラン変更チケットでない場合はNGとする。
これはイベント単位のチェックである。

チェック内容:

- プラン変更チケットがA/U-22/Bを含むこと: チケット単位
- プラン変更チケット以外の各固定費チケットがオン/オフ/ハイ/外を含むこと: チケット単位
- プラン変更チケット以外の各固定費チケットが、イベント種別の通常料金として許可された金額のいずれかであること: チケット単位
- 固定費チケットにA/U-22/Bが混入していないこと: チケット単位

### 8.12 途中参加イベント

イベント名に `全N回` が含まれる場合、途中参加チケットの有無にかかわらずプラン変更チケットを必須とする。さらにチケット名に `第M回から参加` が含まれる場合、途中参加イベントとして通常イベントより先に分岐する。

- プラン変更チケットは全N回イベントで必須とし、第M回から参加チケットが存在しない場合でも、存在しなければエラーにする
- `お申し込み済の方` と `お申し込み済みの方` の両表記を特殊申込済みチケットとして認識する
- プラン変更チケットと特殊申込済みチケットを除いたものを途中参加チケットとする
- 途中参加チケット全体でオン/オフ/ハイ/外が揃っていることを確認する
- 各途中参加チケットの `onlineEnabled` が `true` であることを確認する
- オンラインURL、主催者お知らせ、申込期限は途中参加チケットだけを対象に検証する
- 途中参加チケットの金額は通常の会員プラン別料金チェックの対象外とする
...
イベント一覧を正常取得できた実行では、終了時に最新のCookieを `storageState.json` へ自動保存する。ログイン画面へ転送された場合は、未ログイン状態による上書きを防ぐため保存しない。
`storageState.json` が存在していてもセッションが切れている場合、OSIRO一覧取得時に失敗する。運用担当者は `npm run auth` を再実行する。
イベント一覧では `#eventIndex`、イベント詳細では `#title` と `[name='event_ticket_name']` を画面構造の必須要素として確認する。
一覧の必須要素がない場合または全一覧の取得結果が0件の場合は実行失敗とし、詳細の必須要素がない場合は該当イベントを取得失敗のUNKNOWNとする。
一覧・詳細への遷移ではHTTP応答の有無と成功ステータスを確認する。ページ送り可能な状態で遷移に失敗した場合、同じURLのままの場合、または20ページを超える場合は一覧取得失敗とする。
詳細取得失敗は `CheckResult.status = "unknown"` とし、Slack集計ではNGと分離して `UNKNOWN` 件数と理由を表示する。UNKNOWNが1件以上ある場合は `結果: すべてOK` を出力しない。
Slack投稿は最大3回試行する。HTTP 429、HTTP 5xx、Slackの一時障害エラー、ネットワーク例外を再試行対象とし、認証・権限などの恒久エラーは即時失敗とする。

...
プラン変更チケットが1つあり、それ以外のチケットが1つの場合は固定費イベントとみなす。プラン変更チケット以外が複数の場合は、各チケットがオン/オフ/ハイ/外を含めば固定費イベントとみなす。固定費チケットの金額は通常料金表とは照合せず、チケット間で金額が異なっていてもよい。チケットが2つで片方がプラン変更チケットでない場合はNGとする。
これはイベント単位のチェックである。
